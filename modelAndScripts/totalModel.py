...
#Model Name
PARAMETERS_AND_NAME_MODEL = 'TotalModelPrimoTrainingFinalOnlyLastRegSThirdWayArgMax'
...
# class that create the dataframe from the dataset
...
# class to operate allignment of faces (for training and validation set) and brightness adjustment for data augumentation
...
#instantiate the model to perform allignment
modelPath = ('./landmarks5.dat')

#istatiate the allignement class
allignment = allignment(modelPath)

#class that perform preprocessing (only for training set) with allignment, brigthness, flipping, small rotation
...
#class that perform preprocessing (only for validation set) with allignment
...
#trasformation for training and validation
...
#define the class model for regression part
...
#define the class model for classification part
...
#define the class model for total model
...
#instatiate the regressor part
regModel = SimpleRegressorModel(backboneReg, LastRegressor)

#load weigths for this model
checkpoint1 = torch.load('./BoostingSeed/regressorSecondMediumTraining.pt')
checkpoint1.keys()
regModel.load_state_dict(checkpoint1)

#substitute the first layer of the regressor head, in order to include the concatenation of the 2 classifiers outputs
regModel.LastRegressor[0] = nn.Linear(num_ftrsReg + 162, num_ftrsReg)

#instantiate regressor part of backbone
...
#declare trainable layers and not
count = 0
...
count = 0
for param in model.RegressorHead.parameters(): 
    param.requires_grad = False
    count+=1
print(count)
...
#definintion of the training procedure 
