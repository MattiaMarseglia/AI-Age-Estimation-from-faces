...
PARAMETERS_AND_NAME_MODEL = 'primoClassificatoreArgMax'
...
class dataFrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        samples_weight = self.df.iloc[idx,2]
        ...
        sample = {'image': image, 'age': age, 'samples_weight': samples_weight}
        return sample
...
samples_weight_train = (np.ones(len(X_train))).tolist()
samples_weight_val = (np.ones(len(X_val))).tolist()


df_train = pd.DataFrame({'image':X_train,'age':y_train, 'samples_weight': samples_weight_train})
df_val = pd.DataFrame({'image':X_val,'age':y_val, 'samples_weight': samples_weight_val})
...
##LOAD NEW CHECKPOINT
checkpoint = torch.load('./BoostingSeed/classifierFirstBigTrainingArgMax_updated.pt')
checkpoint.keys()
...
def ClassificationLoss(y_pred,y_true, preds, base_criterion, sample_weights):
    y_true=y_true-1
    #return weighted_loss
    diff = (1 + 0.35*abs(y_true-preds)).to(device)

    y_true=y_true.type(torch.LongTensor).to(device)
    return (base_criterion(y_pred,y_true)*sample_weights*diff).mean()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for sample_batched in tqdm(dataloaders[phase]):
                ...
                samples_weight = sample_batched['samples_weight'].float().to(device)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss = ClassificationLoss(outputs, labels, prediction, base_criterion, samples_weight)
...
def adaboost_clf(train_set, w, model):
    miss = []
    possibleChoise = torch.arange(1,82).unsqueeze(0).to(device)
    for sample_batched in tqdm(train_set):
        inputs = sample_batched['image'].float().to(device)
        labels = sample_batched['age'].float().to(device)
        pred_train_i = model(inputs)
        prob = nn.Softmax(dim=1)(pred_train_i)                    
        # Classifier EV: preds = torch.sum(torch.mul(possibleChoise ,prob),dim=1) -> Classifier argmax: preds = torch.argmax(prob,dim=1)
        preds = torch.argmax(prob,dim=1)
        preds += 1
        # Indicator function
        miss = np.concatenate((miss, [int(x) for x in (torch.logical_and(preds >= labels.data-1, preds <= labels.data+1)).long()]))
...
w = adaboost_clf(train_set, samples_weight_train, model)


PARAMETERS_AND_NAME_MODEL = 'secondoClassificatoreArgMax'


df_train = pd.DataFrame({'image':X_train,'age':y_train, 'samples_weight': w})
df_val = pd.DataFrame({'image':X_val,'age':y_val, 'samples_weight': samples_weight_val})
